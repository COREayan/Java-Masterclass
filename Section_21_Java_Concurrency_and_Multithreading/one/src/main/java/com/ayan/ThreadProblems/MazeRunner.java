package com.ayan.ThreadProblems;
        ...
        while (true) {
            ...
            try {
                ...
                if (maze.searchCell(participant.searchingFor(), newSpot, lastSpot)) {
                    System.out.printf("%s %s at %s!%n", participant.name(), participant.searchingFor(), Arrays.toString(newSpot));
                    break;
...
public class MazeRunner {
    public static void main(String[] args) {
        ...
        while (!adamResults.isDone() && !gracesResults.isDone()) {
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
